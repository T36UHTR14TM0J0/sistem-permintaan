    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Update Data Departement</h5>
                        <form action="<?= base_url('Departement/update/' . $data->id_departement) ?>" method="post" id="edit_form">
                            <div class="form-group">
                                <label for="kode">Kode Departemen:</label>
                                <input type="text" class="form-control" value="<?= $data->id_departement; ?>" id="kode" name="kode" readonly>
                            </div>
                            <div class="form-group">
                                <label for="nama">Nama Departement:</label>
                                <input type="text" class="form-control" id="nama" name="nama" value="<?= $data->nama_departement ?>">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="id_dept_head" name="id_dept_head">
                                    ...
                                    <?php foreach ($dept_heads as $head): ?>
                                        <option value="<?= $head->id_dept_head ?>" <?= $head->id_dept_head == $data->id_dept_head ? 'selected' : '' ?>>
                                            <?= $head->nama_dept_head ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
